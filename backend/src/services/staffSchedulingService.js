...
async function requireScheduleManager(connection, storeId, staffUserId) {
  ...
  if (!["owner", "admin"].includes(String(membership.storeRole || "").toLowerCase())) {
...
async function listEmploymentProfiles(storeId, actorId) {
  ...
  const canManage = ["owner", "admin"].includes(String(membership.storeRole || "").toLowerCase());
...
async function listCalendar(storeId, actorId, from, to) { await requireActiveMembership(pool, storeId, actorId); const [rows] = await pool.query("SELECT * FROM store_business_calendars WHERE store_id=? AND business_date BETWEEN ? AND ? ORDER BY business_date", [storeId,from,to]); return rows; }
