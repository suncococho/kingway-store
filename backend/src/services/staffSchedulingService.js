...
async function assignShift(storeId,actorId,shiftId,input){return withTransaction(async(connection)=>{await requireScheduleManager(connection,storeId,actorId);const staffUserId=positiveId(input.staffUserId,"員工");await requireActiveMembership(connection,storeId,staffUserId,true);const [shifts]=await connection.query("SELECT sh.*,r.period_id,TIME_TO_SEC(TIMEDIFF(sh.ends_at,sh.starts_at))/3600 addedHours FROM work_shifts sh JOIN work_schedule_revisions r ON r.id=sh.revision_id AND r.store_id=sh.store_id WHERE sh.id=? AND sh.store_id=? AND r.status='DRAFT' FOR UPDATE",[shiftId,storeId]);if(!shifts[0])throw httpError("找不到可編輯班次",404);await validateShiftAssignment(connection,storeId,staffUserId,shifts[0]);const [r]=await connection.query("INSERT INTO work_shift_assignments (store_id,shift_id,staff_user_id,status,assigned_by,assignment_reason,is_manual,version) VALUES (?,?,?,'DRAFT',?,?,1,1)",[storeId,shiftId,staffUserId,actorId,text(input.assignmentReason,500)]);return{id:r.insertId,version:1};});}

module.exports={requireActiveMembership,listEmploymentProfiles,upsertEmploymentProfile,listPeriods,createPeriod,listCalendar,upsertCalendar,seedCalendarDefaults,getAvailability,saveAvailability,listTimeOff,createTimeOff,reviewTimeOff,createRevision,getDraft,createShift,attachAssignmentWarnings,validateShiftAssignment,assignShift};
