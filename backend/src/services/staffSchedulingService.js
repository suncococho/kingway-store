...
function attachAssignmentWarnings(shifts,rows){const byShift=new Map(shifts.map(shift=>[Number(shift.id),shift]));for(const row of rows){const shiftId=Number(row.shiftId),shift=byShift.get(shiftId);if(!shift)continue;let code=null,message=null;if(!Number(row.availabilitySubmitted)){code="AVAILABILITY_NOT_SUBMITTED";message="員工尚未提交可排班時間";}else if(Number(row.notPreferred)){code="NOT_PREFERRED";message="員工已標記此時段為不希望排班";}else if(!Number(row.availableOrPreferred)){code="OUTSIDE_AVAILABILITY";message="員工未提供此時段可排班";}if(code){shift.assignmentWarnings.push({shiftId,staffUserId:Number(row.staffUserId),displayName:row.displayName,code,message});if(!shift.warnings.includes(message))shift.warnings.push(message);}if(row.contractedWeeklyHours!==null&&row.contractedWeeklyHours!==undefined&&Number(row.scheduledHours)>Number(row.contractedWeeklyHours)){message="已超過契約週工時";shift.assignmentWarnings.push({shiftId,staffUserId:Number(row.staffUserId),displayName:row.displayName,code:"CONTRACTED_WEEKLY_HOURS_EXCEEDED",message});if(!shift.warnings.includes(message))shift.warnings.push(message);}}return shifts;}
