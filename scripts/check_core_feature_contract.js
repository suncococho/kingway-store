...
const infrastructureContracts=array(manifest.infrastructureContracts,"infrastructureContracts");
unique(infrastructureContracts.map(x=>x.contractId),"infrastructure contract");
const backendValidation=infrastructureContracts.find(x=>x.contractId==="backend-read-only-validation");
if(!backendValidation)fail("backend read-only validation infrastructure contract missing");
else{
 if(backendValidation.environmentVariable!=="BACKEND_VALIDATION_MODE")fail("backend validation environment variable mismatch");
 if(backendValidation.enabledValue!=="read-only"||backendValidation.defaultValue!=="off")fail("backend validation mode/default mismatch");
 sameSet(array(backendValidation.allowedHttpMethods,"backendValidation.allowedHttpMethods"),["GET","HEAD","OPTIONS"],"backend validation allowed methods");
 sameSet(array(backendValidation.blockedHttpMethods,"backendValidation.blockedHttpMethods"),["POST","PUT","PATCH","DELETE"],"backend validation blocked methods");
 for(const rel of [...array(backendValidation.sourceFiles,"backendValidation.sourceFiles"),...array(backendValidation.testFiles,"backendValidation.testFiles")])exists(rel,"backend validation contract");
 const image=backendValidation.immutableImage||{};
 for(const rel of [image.dockerfile,image.dockerignore])exists(rel,"backend immutable image");
 if(image.baseImage!=="node:20-alpine"||image.installCommand!=="npm ci --omit=dev --no-audit --no-fund")fail("backend immutable image dependency contract mismatch");
 if(JSON.stringify(image.command)!==JSON.stringify(["node","src/server.js"]))fail("backend immutable image command mismatch");
 if(image.revisionLabel!=="org.opencontainers.image.revision"||image.runtimeUser!=="node")fail("backend immutable image revision/user mismatch");
 const runtime=read("backend/src/runtime/validationMode.js"),middleware=read("backend/src/middleware/readOnlyValidation.js"),server=read("backend/src/server.js"),database=read("backend/src/db.js"),dockerfile=read("backend/Dockerfile"),dockerignore=read("backend/.dockerignore");
